...
class CustomHelper extends Helper {
    ...
    public function nestedLinks($links, $options = array(), $depth = 1) {
        ...
        foreach ($links AS $link) {
            ...
            if (empty($this->request->params['locale']) && Configure::read('Site.locale') == 'fra') {
                foreach ($link as $l) {
                    if (!empty($l[0]['locale'])) {
                        foreach ($l as $t) {
                            if ($t['locale'] == 'fre') {
            ...
            $linkOutput = $this->Html->link($title, $link['Link']['link']);
            if (isset($link['children']) && count($link['children']) > 0) {
                $linkOutput = $this->Html->link($title . '<b class="caret"></b>', $link['Link']['link'], array('class' => $options['toggle'], 'data-toggle' => $options['dropdownClass'], 'escape' => false));
                if (empty($linkAttr['class'])) {
                    $linkAttr['class'] = '';
                }
                $linkAttr['class'] .= ' ' . $options['dropdownClass'];
                if (!empty($link['children'][0]['Link']['extras'])) {
                    $linkOutput .= $link['children'][0]['Link']['extras'];
                } else {
                    $linkOutput .= $this->nestedLinks($link['children'], $options, $depth + 1);
                }
            }
            $outpulinks = $this->Html->tag('li', $linkOutput, $linkAttr);
            $output .= $outpulinks;
        }
        if ($output != null) {
